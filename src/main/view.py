...
def auth_qq():
    ...
    if not code:
        ...
        get_open_id_url = "https://graph.z.qq.com/moc2/me?access_token=%s" % (access_token)
        res = urllib.request.urlopen(get_open_id_url).read()
        return json.dumps({'resp': str(v), 'access_token': str(access_token), 'res': res.decode('utf8')})
